...
export function getReportIndeces(year: number) {
  return new Promise<string[]>((resolve, reject) => {
    ...
    request(
      ...
      (err, res, body) => {
        ...
        const aTags = (document.getElementsByTagName(
          "a"
...
export function getReportViaIndex(index: string) {
    ...
    request(
      ...
      (err, res, body) => {
        if (!err && res.statusCode == 200) {
          ...
          const liTags = (document.getElementsByTagName(
            "li"
          ...
          if (liTags.length > 0) {
            for (const liTag of liTags) {
              ...
              const newsTitle = rawTitle
                .replace(newsDate, "")
                .replace(newsOfficial ? "［公表］" : "", "");

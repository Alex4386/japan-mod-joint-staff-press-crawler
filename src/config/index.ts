const config = {
  ...
  generateMainPage: (year: number) =>
    year == new Date().getFullYear()
      ? config.mainPage
      : config.mainBaseURL + year.toString() + ".htm",

  pressAPIBasePage: "https://www.mod.go.jp/js/tpl/Press/",
  generateAPIRequest: (year: number, month: number) =>
    config.pressAPIBasePage +
    year +
    "/" +
    year +
  ...
  userAgent:
    "Mozilla/5.0 (X11; CrOS x86_64 10066.0.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36 ProfWonKimBrowser/127.13"
};
